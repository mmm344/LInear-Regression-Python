...
OLS_summary_df.loc[abs(OLS_summary_df['Residuals'])>= resid_threshhold*np.std(residuals.Residuals), 'Possible Outlier?'] = 'Yes'
OLS_summary_df.loc[abs(OLS_summary_df['Residuals'])< resid_threshhold*np.std(residuals.Residuals), 'Possible Outlier?'] = 'No'
...
plt.plot(x.Projected_Revenue, y_hat_sample1+resid_threshhold*np.std(residuals.Residuals), 'r', alpha = 1)
plt.plot(x.Projected_Revenue, y_hat_sample1-resid_threshhold*np.std(residuals.Residuals), 'r', alpha = 1)
